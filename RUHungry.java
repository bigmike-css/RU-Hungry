...
public class RUHungry {
    ...
    StdIn.setFile(inputFile);
        ...
        int n = StdIn.readInt();
        categoryVar = new String[n];
        menuVar = new MenuNode[n];
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < d; j++) {
                ...
                Dish dish = new Dish(categoryVar[i], dishName, id);
    ...
    if (newNode == null || stockVar == null) {
        return;
    }

    int id = newNode.getIngredient().getID();
    int inta = id % stockVarSize;

    if (stockVar[inta] != null) {
        newNode.setNextStockNode(stockVar[inta]);
    }

    stockVar[inta] = newNode;
}
    ...
    if (stockVarSize == 0 || stockVar == null) {
        return null;
    }

    int instant = ingredientID % stockVarSize;
    StockNode point = stockVar[instant];

    while (point != null) {
        if (point.getIngredient().getID() == ingredientID) {
            return point;
        }
        point = point.getNextStockNode();
    }
	...
	// WRITE YOUR CODE HERE
    StockNode amount = null;
        if (ingredientName != null && !ingredientName.isEmpty()) {
            amount = findStockNode(ingredientName);
        } else if (ingredientID != -1) {
            amount = findStockNode(ingredientID);
        }

        if (amount != null) {
            amount.getIngredient().updateStockLevel(stockAmountToAdd);
        } else {
            String item = ingredientName; 
            double price = 0.0;
            Ingredient n = new Ingredient(ingredientID, item, stockAmountToAdd, price);
            StockNode i = new StockNode(n, null);
            int iid = ingredientID % stockVarSize;
            i.setNextStockNode(stockVar[iid]);
            stockVar[iid] = i;
        }
    ...
    for (MenuNode category : menuVar) {
        MenuNode ptr = category;

        while (ptr != null) {
            Dish currentDish = ptr.getDish();
            double totalCost = 0;
            ...
            double prof = price - totalCost;

            currentDish.setPriceOfDish(price);
            currentDish.setProfit(prof);
    ...
    public void createStockHashTable (String inputFile){
	    ...
	    // WRITE YOUR CODE HERE
            ...
            while (!StdIn.isEmpty()) {
                ...
                Ingredient ingredient = new Ingredient(id, item, quantity, price);
	...
	// WRITE YOUR CODE HERE
    TransactionNode newTNode = new TransactionNode(data, null);

    if (transactionVar == null) {   transactionVar = newTNode;
    } else {
        TransactionNode point = transactionVar;
        while (point.getNext() != null) {
            point = point.getNext();
        }
        point.setNext(newTNode);
    }
	...
	// WRITE YOUR CODE HERE
    MenuNode name = findDish(dishName);
    if (name == null) {
        return false;
    }

    Dish item = name.getDish();

    for (int ingredientID : item.getStockID()) {
        StockNode a = findStockNode(ingredientID);
        if (a == null || a.getIngredient().getStockLevel() < numberOfDishes) {
            return false;
        }
    }
    ...
    if (checkDishAvailability(dishName, quantity)){
        ...
        int[] list = findDish(dishName).getDish().getStockID();
        for (int i = 0; i < list.length; i++){
            updateStock(null, list[i], -1 * quantity);
        }
    } else {
        TransactionData data = new TransactionData("order", dishName, quantity, 0, false);
        addTransactionNode(data);
        MenuNode pointer = findDish(dishName).getNextMenuNode();
        if (pointer != null){
            orderHelper(pointer.getDish().getName(), quantity, dishName);
        } else {
            int item = findCategoryIndex(findDish(dishName).getDish().getCategory());
            orderHelper(menuVar[item].getDish().getName(), quantity, dishName);
        }
    }

        
    }
    private void orderHelper (String dishName, int quantity, String first){
        if (dishName.equals(first)) return;
        if (checkDishAvailability(dishName, quantity)){
            TransactionData data = new TransactionData("order", dishName, quantity, quantity * findDish(dishName).getDish().getProfit(), true);
            addTransactionNode(data);
            int[] list = findDish(dishName).getDish().getStockID();
            ...
            TransactionData data = new TransactionData("order", dishName, quantity, 0, false);
            addTransactionNode(data);
            MenuNode point = findDish(dishName).getNextMenuNode();
            if (point != null){
                orderHelper(point.getDish().getName(), quantity, first);
            } else {
                int item = findCategoryIndex(findDish(dishName).getDish().getCategory());
                orderHelper(menuVar[item].getDish().getName(), quantity, first);
            }
        }
    }
    ...
    while (point != null) {
        TransactionData data = point.getData();
        prft += data.getProfit();
        point = point.getNext();
    }
    ...
    if (price > 50) {
        if (stock != null && stock.getIngredient().getStockLevel() >= quantity) {
            qty = true;     
            updateStock(ingredientName, -1, -quantity);
        }
    }

    TransactionData t = new TransactionData("donation", ingredientName, quantity, 0.0, qty);
    ...
    StockNode costa = findStockNode(ingredientName);
    if (costa == null) {
        TransactionData info = new TransactionData("restock", ingredientName, quantity, 0, false);
        addTransactionNode(info);
        return;
    }

    double ingredientcost = costa.getIngredient().getCost();
    double cost = ingredientcost * quantity;
    double money = profit();
    boolean restock = money >= cost;
    double profit = 0;
    ...
    TransactionData data = new TransactionData("restock", ingredientName, quantity, profit, restock);
    addTransactionNode(data);
    ...
    */
    ...
    leftQueueVar = new Queue<>(); // Initialize leftQueueVar

    while (!waitingQueue.isEmpty()) {
        Party nextParty = waitingQueue.dequeue();

        // Attempt to find an available table for the next party
        boolean seated = false;
        for (int i = 0; i < tables.length; i++) {
            if (tables[i] == null && tableSeats[i] >= nextParty.getNumberOfPeople()) {
                // Found an available table, seat the party
                tables[i] = nextParty;
                seated = true;
                break;
            }
        ...
        if (!seated) {
            // No available table for the next party, kick out a party to make room
            for (int i = 0; i < tables.length; i++) {
                if (tableSeats[i] >= nextParty.getNumberOfPeople()) {
                    // Remove the current party, add them to leftQueueVar
                    leftQueueVar.enqueue(tables[i]);
                    // Seat the next party in line
                    tables[i] = nextParty;
                    seated = true;
                    break;
                }
        ...
        if (!seated) {
            // Unable to seat the party, add them back to the waitingQueue
            waitingQueue.enqueue(nextParty);
        }
    }

    // Move all parties from tables to leftQueueVar
    for (int i = 0; i < tables.length; i++) {
        if (tables[i] != null) {
            leftQueueVar.enqueue(tables[i]);
            tables[i] = null; // Clear the table
        }
    }
